...
#if defined(NDEBUG)
# error "Montero cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7aefaf2f227afbfd996d2cbf1d3e335744e593e7af7f351f062107b8f4ca5363");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Montero: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Montero Signed Message:\n";
...
{
    // Montero: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Montero: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Montero: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Montero: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Montero: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Montero: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa1db3dfbdf84a5a48e0c1f92069897a92f2d0a15bf15d31528450de30d896476");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Cluj-Napoca 08/Oct/2021 Bojan Sebastian, Montero Visionary , sell his soul at 5 AM";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042a13ff7677764ce575005ce7115d703265c44fbd62f343a0819a5c8e67da8e7d62939dbb74081c3ba9146e11d2a971bd09a04d5fb8df53046ff6c5536ed1709e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1633659026;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085091491;

        if (fTestNet)
        {
            block.nTime    = 1633658995;
            block.nNonce   = 387165305;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xf314a9da7be94b5b64467bf44920149991a324ef6b848001d11087678efbc9e5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc9, 0xb4, 0xdb }; // Montero: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MonteroMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MonteroMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MonteroMiner(CWallet *pwallet)
{
    printf("MonteroMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Montero-miner");
    ...
    try { loop {
        ...
        printf("Running MonteroMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MonteroMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MonteroMiner, pwallet));
}
